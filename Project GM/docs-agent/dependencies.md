...
|------------|----------|-------|
| MySQL 8 (`mysql-connector-j`, `flyway-mysql`) | System of record, schema managed by Flyway (`V1`-`V6` under `src/main/resources/db/migration`) | Local dev DB `project_gm_local`, test DB `project_gm_test`, both need the `project_gm_user` role. No Postgres anymore (migrated 2026-09-20). |
