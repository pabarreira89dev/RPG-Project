...
### MVP v0.3 — Inventario/`Item` implementado (2026-09-22)

Primer punto de `MVP v0.3.md` implementado, siguiendo el mismo patrón "catálogo global + estado por sesión" ya usado para Npc/Relationship y Quest/QuestState: `pab.rpg.domain.entity.ItemTemplate` (catálogo global, como `Location`/`Npc`: `id`, `code` único, `name`, `description`) y `pab.rpg.domain.entity.Item` (instancia por sesión: `id`, `sessionId`, `templateId`, `ownerId`/`locationId` — exactamente uno de los dos no-nulo, `quantity`, `durability` nullable sin uso todavía). Migración `V7__create_item_template_and_item.sql`: crea `item_template` con 3 objetos seed (`rusty_dagger`, `healing_herbs`, `old_coin_pouch`) y `item` con `CHECK` de propietario-xor-ubicación (mismo patrón que el `CHECK` de `combat_participant` para character_id/npc_id).

`ItemService`/`ItemServiceImpl` (`pab.rpg.service`): `getInventory(sessionId)` (objetos del personaje), `getItemsAtLocation(sessionId, locationId)` (objetos en el suelo de una localización), `pickUpItem(sessionId, itemId)` (valida que el objeto siga sin dueño y esté en la localización actual del jugador, lo asigna al `Character` y añade evento `ITEM_ACQUIRED` vía `GameEventService` — cumple el invariante del TDD §15 "ningún objeto aparece sin evento `ITEM_ACQUIRED`"), `seedInitialItems(sessionId)` (siembra los 3 objetos seed sin dueño en sus localizaciones fijas). `GameSessionServiceImpl.createSession` llama a `seedInitialItems` justo después de guardar la sesión, así toda partida nueva empieza con objetos para encontrar.

Endpoint nuevo `ItemController` en `/api/v1/sessions/{sessionId}/items` (mismo patrón que `NpcController`/`QuestController`, no embebido en `GameSessionResponse` pese a que el TDD §9.2 lo sugiere así — se prioriza consistencia con cómo ya se implementaron NPCs/misiones como sub-recursos propios): `GET` devuelve `InventoryResponse(inventory, atLocation)` (inventario del personaje + objetos visibles en la localización actual); `POST /{itemId}/pick-up` recoge un objeto. Excepciones nuevas `ItemNotFoundException`→404 `ITEM_NOT_FOUND` e `ItemNotAllowedException`→422 `ITEM_NOT_ALLOWED` (objeto ya recogido o en otra localización), registradas en `GlobalExceptionHandler` siguiendo el mismo patrón que Combat/Quest.
...
(2026-09-24) El emisor externo de JWT que faltaba para `cloud` ya existe: nuevo repo `Project GM Auth/` (Spring Authorization Server independiente, ver su propio `docs-agent/`). Persiste cuentas (`AppUser`, UUID propio generado por Hibernate) en su propia base MySQL. Un `OAuth2TokenCustomizer` fuerza `sub`=UUID de la cuenta (no el username) y añade el claim `aud` configurado — ese es el único punto de acoplamiento real con este repo (`JWT_ISSUER_URI`/`JWT_AUDIENCE` deben coincidir exactamente con `AUTH_ISSUER_URI`/`AUTH_JWT_AUDIENCE` de `Project GM Auth`). Sin cambios de código en este repo.

(2026-09-25) La app Android originalmente iba a autenticarse vía Authorization Code+PKCE (AppAuth, Custom Tabs) contra las páginas Thymeleaf de login/registro — abandonado tras probar en un dispositivo físico real: el navegador (Custom Tabs) retenía su propia cookie de sesión con `Project GM Auth`, así que tras el primer login la app "recordaba" la sesión sin volver a pedir credenciales, y esta versión de Spring Authorization Server no implementa `prompt=login` para forzar re-autenticación. Se sustituyó por un grant type OAuth2 custom `password` (formularios nativos de usuario/contraseña en la propia app, sin navegador), verificado funcionando de extremo a extremo en dispositivo real — ver `Project GM Auth/docs-agent/architecture.md` y `MVP v0.4.md`. Las páginas Thymeleaf `/login`/`/register` se conservan solo para pruebas manuales/admin.
...
No queda ningún punto pendiente del "Orden recomendado de trabajo" original (1–10), ni el punto 13 (observabilidad) ni el punto 12 (seguridad JWT, ver detalle arriba). El vertical slice MVP v0.2 tiene interpretación de texto libre en sus tres flujos principales, observabilidad mínima y autenticación JWT (con bypass de identidad fija/`X-Dev-Player-Id` en local/test). El punto 12 ya está verificado: compila y todos los tests pasan con el perfil `test` (incluido el `ApplicationTests` de contexto completo). En `cloud` sigue haciendo falta decidir/configurar un proveedor de identidad JWT real (issuer+audience) antes de desplegar.

### Punto 14 implementado: E2E completo

`Project GM automatics/` ya cubría el recorrido de 10 pasos de la sección 15 del TDD (`recorrido_completo.feature`: crear partida, acción libre, evento/estado, reinicio real del proceso, recuperación, combate y misión con rama, todo con persistencia verificada). Lo que faltaba era cobertura E2E **aislada** por endpoint (fuera del recorrido feliz encadenado) y de los caminos de rechazo — se añadieron 4 features nuevas, reutilizando los steps ya existentes de `GameSessionSteps.java` y añadiendo solo los que faltaban:

- `combate.feature`: iniciar combate válido, iniciar combate contra un NPC fuera de la localización actual (422 `COMBAT_NOT_ALLOWED`), consultar combate activo sin ninguno (404 `COMBAT_NOT_FOUND`), combate completo standalone (sin reinicio de la app, a diferencia del que ya existía dentro de `recorrido_completo.feature`).
- `misiones.feature`: misiones visibles de una sesión nueva (lista vacía — `getVisibleQuests` solo devuelve misiones con `QuestState`, no todo el catálogo), iniciar misión, avanzar con una decisión válida (completa la misión) y con una decisión inexistente (422 `QUEST_TRANSITION_NOT_ALLOWED`).
- `npcs.feature`: listar NPCs de la localización actual, relación inicial neutra (0), y que una acción SOCIAL exitosa la cambia (evento `RELATIONSHIP_CHANGED`).
- `autenticacion.feature`: sin cabecera `X-Dev-Player-Id` se usa la identidad de desarrollo fija por defecto (crear + volver a consultar la misma sesión sin cabecera, ambas peticiones resuelven al mismo jugador por defecto — prueba end-to-end del `DevelopmentIdentityFilter` del punto 12), y que un jugador no puede consultar la sesión de otro (ya existía el escenario equivalente en `sesiones_de_juego.feature`, se repite aquí en el contexto específico de identidad/seguridad).

Cambios en `GameSessionSteps.java`: `el_jugador_ha_iniciado_combate_contra` ahora solo parsea `combatId` si la respuesta es 200 (antes asumía siempre éxito y lanzaba una excepción Java poco clara si se reutilizaba en un escenario de rechazo); se añadieron alias `@When` sobre pasos que antes solo eran `@Given` (`el jugador inicia combate contra`/`el jugador inicia la misión`, mismo método que la versión `@Given`, sin duplicar lógica) más los pasos nuevos exclusivos de las features nuevas (consultar combate activo/NPCs/misiones visibles, relación exacta, crear/consultar sesión sin cabecera de identidad).

No se ha ejecutado la suite en esta sesión (requiere "Project GM" levantado en local con MySQL arriba); pendiente que el usuario la corra para confirmar que las 4 features nuevas pasan.

**(2026-09-21) Ejecutada y verificada**: las 8 features (26 escenarios) de `Project GM automatics` pasan al 100%, incluida `recorrido_completo.feature` con el reinicio real del proceso. Se encontró y corrigió un problema real en el propio mecanismo de reinicio: `AppLifecycle` relanza "Project GM" con `ProcessBuilder`, que **hereda las variables de entorno del proceso que ejecuta `mvn test`**, no las de la terminal interactiva del usuario. Si esa terminal no tiene cargado `Project GM/.env.local` (con `OPENAI_ENABLED`/`OPENAI_API_KEY`/etc., sin valores por defecto en `application-local.yml`), el proceso relanzado falla al arrancar con un `BeanDefinitionStoreException` engañoso (el mensaje apunta a una clase cualquiera del classpath scan, p.ej. `OpenAiMasterAdapter.class`, pero la causa real 2 niveles más abajo es `PlaceholderResolutionException: Could not resolve placeholder 'OPENAI_ENABLED'`). **Antes de correr `mvn test` en `Project GM automatics`, cargar `.env.local` en esa misma sesión de PowerShell** (mismo patrón que para arrancar "Project GM" manualmente). Detalle completo en la nota de depuración de memoria de usuario.

Con esto no queda ningún punto pendiente del "Orden de implementación" del TDD MVP v0.2 (1–14). Quedan mejoras de alcance fuera del TDD original, ya recogidas en `MVP v0.3.md`: combate más allá de atacar (moverse/defenderse/usar objeto, sistema de armas/objetos — GDD sección 11), diálogo real que use `NpcKnowledgeFact` (todavía sin llamador), inventario/`Item`, reglas de muerte, memoria de conversación, reintentos ante throttling de OpenAI y límites de uso por usuario.
