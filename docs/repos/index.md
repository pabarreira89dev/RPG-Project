...
|------|------|---------|-----------|----------|
| Project GM | #back #api #java #maven | Spring Boot game engine + API: owns all game state, rules, persistence; isolates the AI narrator behind `MasterAdapter` | (none — the only backend service) | [Project GM/docs-agent/index.md](../../Project%20GM/docs-agent/index.md) |
...
Both repos are plain subfolders of this same workspace (not independent git clones under a gitignored
`repos/` folder as in the generic kit) — there is a single `.git` for the whole workspace.
