...
**Estado (2026-09-26)**: proyecto Android andamiado y compilando (`Project GM Android/`, Kotlin/Jetpack Compose/Material 3, capa de red con Retrofit + kotlinx.serialization). Autenticación (flavor `cloud`) verificada de extremo a extremo en dispositivo físico real (login, registro y refresh). Gestión de partidas y narración/acción libre implementadas (`gradlew assembleLocalDebug assembleCloudDebug` en verde), pendientes de verificación manual en emulador/dispositivo. El resto de pantallas (NPCs, inventario, misiones, combate) sigue sin empezar.
...
|Gestión de partidas          |✅ Implementado (2026-09-25), pendiente de verificación manual — `SessionRepository`/`SessionListViewModel`/`SessionListScreen` (listar con pull-to-refresh, crear con diálogo de nombre+confirmación), nueva `SessionDetailScreen`/`SessionDetailViewModel` (ver detalle, borrar con confirmación) y `NavHost` (Navigation-Compose) conectando login/registro/lista/detalle. Borrado lógico nuevo en el backend (`DELETE /api/v1/sessions/{sessionId}`, columna `deleted_at`, migración V9) |Pantalla de lista de partidas del jugador + creación de partida nueva                             |Sessions|
|Narración y acción libre     |✅ Implementado (2026-09-26), pendiente de verificación manual — nueva `ActionRepository`/`NarrationViewModel`/`NarrationScreen` (sustituye a `SessionDetailScreen`, que se eliminó: la pantalla de chat es ahora el destino directo al pulsar una sesión, con el borrado movido a su barra superior). Solo texto libre, sin selector de `actionType`. Historial de turnos persistido localmente con Room (primer uso de Room/KSP en el proyecto; ver `data/local/`). Maneja 409 `STALE_SESSION_VERSION` (resincroniza versión + botón "Reintentar" reutilizando el mismo `idempotencyKey`), 422 `ACTION_NOT_ALLOWED` y 503 `OPENAI_UNAVAILABLE` (burbuja de sistema, sin reintento, la acción ya se resolvió en el motor) |Actions|
