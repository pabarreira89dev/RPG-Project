...
|Autenticación                |🔶 Parcial (2026-09-24) — `DevIdentityInterceptor` añade `X-Dev-Player-Id` solo si se configura un playerId (por defecto usa la identidad dev fija del backend); sin JWT real para `cloud` todavía |La app obtiene/adjunta el token (o el header `X-Dev-Player-Id` en desarrollo) en cada petición    |Nota de autenticación en cabecera del contrato|
